...
`test/fcose_test.dart`. Short deterministic runs agree to about 1e-14;
the ones that iterate longest (`iteration-floor`, `post-growth-cooling`,
`tree-reduction-cooling`) drift up to a third of a pixel, which is the
accumulated difference between JavaScript and Dart evaluation of the same
nonlinear ticks rather than a behavioral gap.

## Known divergences

`draft-nested-compound.json` still differs: the port mirrors the layout on x
and puts the edgeless child `quiet` five pixels off the center upstream gives
it. Both belong to the open compound-spectral parity work.

`draft-disconnected.json` is the opposite case — upstream is the one that is
...
kept as a reproducer rather than as an expectation.
